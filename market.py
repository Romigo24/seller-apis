...
def get_product_list(page, campaign_id, access_token):
    ...
    Аргументы:
        page: Токен страницы для постраничной навигации. Позволяет 
        ...
        access_token: Токен доступа для аутентификации при запросе к API.

    Возвращает список товаров в соответствии с параметрами запроса. 
...
def update_stocks(stocks, campaign_id, access_token):
    ...
    Аргументы:
        stocks: Список обновленных остатков товара, каждый элемент должен 
                    иметь структуру, соответствующую требованиям API, например, 
                    список SKU и их остатков.
        campaign_id: Идентификатор рекламной кампании, к которой 
                         относятся обновления остатков товаров.
        access_token: Токен доступа для аутентификации при обращении к API.

    Возвращает ответ от API, содержащий результаты операции обновления.
"""
    endpoint_url = "https://api.partner.market.yandex.ru/"
    headers = {
        "Content-Type": "application/json",
...
def update_price(prices, campaign_id, access_token):
    ...
    Аргументы:
        prices: Список обновленных цен товаров. Каждый элемент должен иметь структуру, 
                       соответствующую требованиям API, включая SKU и новую цену.
        campaign_id: Уникальный идентификатор рекламной кампании, к которой 
...
def get_offer_ids(campaign_id, market_token):
    ...
    Аргументы:
        campaign_id: Уникальный идентификатор рекламной кампании, для
                       которой нужно получить артикулы товаров.
        market_token: Токен доступа для аутентификации при запросе
                        к API Яндекс.Маркета.

    Возвращает список артикулов товаров, относящихся к указанной
...
def create_stocks(watch_remnants, offer_ids, warehouse_id):
    ...
    Аргументы:
        watch_remnants: Список остатков товаров, полученных из
            другого источника, где каждый элемент является словарем, 
            содержащим данные о товаре (например, "Код" и "Количество").
        offer_ids: Список артикулов (SKU), которые должны быть
            учтены при формировании остатков.
        warehouse_id: Идентификатор склада, к которому относятся
            формируемые остатки.

    Возвращает список остатков, форматированный для передачи в API Яндекс.Маркет. Каждый элемент списка — это словарь,
...
def create_prices(watch_remnants, offer_ids):
    ...
    Аргументы:
    watch_remnants: Список с остатками товаров, где каждый 
    элемент представляет собой словарь с данными о товаре, 
    включая "Код" и "Цена".
    
    offer_ids: Список артикулов (SKU), для которых необходимо 
    создать объекты цен.

    Возвращает список объектов цен, каждый из которых содержит идентификатор товара, 
