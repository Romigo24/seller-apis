...
def get_product_list(last_id, client_id, seller_token):
    ...
    Аргументы:
        last_id (str): Идентификатор последнего товара. Используется для
            постраничного вывода. Если это первый вызов функции, можно
            передать 0 или None.
        client_id (str): Уникальный идентификатор клиента для аутентификации
            при обращении к API Ozon.
        seller_token (str): Токен продавца для доступа к защищённым ресурсам
            API Ozon.

    Возвращает:
        list: Список товаров, возвращенный API Ozon. Каждый товар представлен
...
def get_offer_ids(client_id, seller_token):
    ...
    Возвращает:
        list: Список артикулов (offer_id) товаров, доступных в магазине Ozon.
    """
    last_id = ""
    product_list = []
    while True:
...
def update_price(prices: list, client_id, seller_token):
    ...
    Возвращает:
        dict: ответ от API Ozon, содержащий информацию о результате работы функции.
    """
...
def update_stocks(stocks: list, client_id, seller_token):
    ...
    Аргументы:
        stocks (list): Список словарей с информацией об остатках товаров. 
            ...
            - offer_id (str): Идентификатор предложения товара.
            - quantity (int): Новое количество товара на складе.
            Пример: [{"offer_id": "123", "quantity": 10}, {"offer_id": "456", "quantity": 5}].
        
        client_id (str): Уникальный идентификатор клиента для аутентификации 
            при обращении к API Ozon.
        
        seller_token (str): Токен продавца для доступа к защищённым ресурсам API Ozon.

    Возвращает:
        dict: ответ от API Ozon, содержащий информацию о результате операции.
    """
...
def download_stock():
    ...
    Возвращает:
        list: список остатков товаров, где каждый элемент представляет 
...
def create_stocks(watch_remnants, offer_ids):
    ...
    Аргументы:
        watch_remnants (list): Список словарей, где каждый словарь содержит
            информацию о товаре, включая его код и количество остатка.
        offer_ids (set): Список идентификаторов предложений (offer_id), для
            которых необходимо создать остатки.

    Возвращает:
        list: cписок словарей с информацией об остатках на продукцию, где каждый
...
def create_prices(watch_remnants, offer_ids):
    ...
    Аргументы:
        watch_remnants (list): Список словарей, представляющих остатки часов, 
                            где каждый словарь содержит информацию о модели, 
                            включая "Код" и "Цена".
        offer_ids (list): Множество идентификаторов предложений, по которым 
                     будет производиться фильтрация остатков.

    Возвращает:
        prices (list): Список словарей, где каждый словарь содержит информацию о цене 
...
def price_conversion(price: str) -> str:
    ...
    Аргументы:
        price (str): Строка, содержащая цену, которая может включать 
